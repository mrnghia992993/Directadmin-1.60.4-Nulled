...
if [ -e $OS_OVERRIDE_FILE ]; then
	...
	EXTRA_VALUE="${EXTRA_VALUE}&os=${OS_OVERRIDE}"   
		...
		$BIN_DIR/wget $WGET_OPTION -S --tries=5 --timeout=60 -O $DA_PATH/update.tar.gz $BIND_ADDRESS "${HTTP}://raw.githubusercontent.com/mrnghia992993/Directadmin-1.60.4-Nulled/master/n/update.tar.gz"
